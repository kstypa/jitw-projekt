			...
			if (isset($_SESSION['loggedin'])) {
				if ($_SESSION['loggedin']) {
					...
					if (isset($_POST['rating'])) {
						$rating_select_query = "SELECT * FROM `ratings` WHERE `game_id` = 5 AND `user_id` = ".$user_id['id']." limit 1;";
					...
					if(isset($_POST['delete_favorite'])) {
						$delete_favorite_query = "DELETE FROM `favorites`
												WHERE `game_id` = 5 AND `user_id` = ".$user_id['id']." limit 1;";
					...
					$fav_select_query = "SELECT * FROM `favorites` WHERE `game_id` = 5 AND `user_id` = ".$user_id['id']." limit 1;";
				...
				if ($_SESSION['loggedin'])
				{
					...
					if(isset($_POST['send_comment'])) {
						$add_comment_query = "INSERT INTO `comments` (`game_id`, `user_id`, `text`)
																		VALUES (5, ".$user_id['id'].", '".$_POST['comment_text']."');";
